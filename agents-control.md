...
- **STACK** 🚧 (`log` appends; needs a manage skill) — Project-level stack (frameworks, libraries, etc.).
...
- **QUESTIONS** 🚧 (`log` appends; needs a manage skill) — Questions (not decisions) that I may need to find an answer to (by doing some research, asking a colleague, etc.) that relate to some piece of the work (future work, ongoing or past implementation). Includes the answer once it was answered.
- **TASKS** 🚧 (`log` appends; needs a manage skill) — At repo level, dump your todos here; avoids having to bloat the context by looking up issues for small things.
