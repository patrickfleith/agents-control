...
_Status added later — ✅ = file operations already implemented by a skill (skill named) · 🚧 = not yet, or only partial (e.g. `log` can append entries, but a dedicated skill to **manage** them — edit, resolve, check off — is still needed)._
...
- **STACK** 🚧 (`log` appends; needs a manage skill to revise/supersede choices) — Project-level stack (frameworks, libraries, etc.).
...
- **QUESTIONS** 🚧 (`log` appends; needs a manage skill to fill in answers) — Questions (not decisions) that I may need to find an answer to (by doing some research, asking a colleague, etc.) that relate to some piece of the work (future work, ongoing or past implementation). Includes the answer once it was answered.
- **TASKS** 🚧 (`log` appends; needs a manage skill to check off/complete/remove) — At repo level, dump your todos here; avoids having to bloat the context by looking up issues for small things.
